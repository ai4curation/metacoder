...
@pytest.mark.llm
@pytest.mark.parametrize("coder_name,coder_class", [
    ("claude", ClaudeCoder),
    ("goose", GooseCoder),
])
@pytest.mark.parametrize("product_id,expected_product", [
    (1, "chocolate chip cookies"),          # odd number
    (2, "salt and vinegar potato chips"),  # even number
])
def test_coder_mcp_product_lookup(runner, mcp_config_file, goose_mcp_config_file, coder_name, coder_class, product_id, expected_product):
...
def test_coder_mcp_multiple_lookups(runner, mcp_config_file, goose_mcp_config_file, coder_name, coder_class):
